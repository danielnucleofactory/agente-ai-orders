<div>
        ...
        <table class="border-collapse table-auto w-full whitespace-no-wrap bg-white table-striped relative">
            ...
            <tbody>
                @forelse ($vendors as $vendor)
                    <tr>
                        <td class="border-dashed border-t border-gray-200 px-3 py-2">{{ $vendor->name }}</td>
                        <td class="border-dashed border-t border-gray-200 px-3 py-2">{{ $vendor->email ?? '--' }}</td>
                        <td class="border-dashed border-t border-gray-200 px-3 py-2">{{ $vendor->contact_person ?? '--' }}</td>
                        <td class="border-dashed border-t border-gray-200 px-3 py-2">{{ $vendor->vendor_direccion ?? '--' }}</td>
                        <td class="border-dashed border-t border-gray-200 px-3 py-2">{{ $vendor->vendor_telefono ?? '--' }}</td>
                        ...
                        <td class="border-dashed border-t border-gray-200 px-3 py-2">
                            <div class="flex space-x-2">
                                ...
                                <button wire:click="$emit('confirmDelete', {{ $vendor->id }})" class="text-red-600 hover:text-red-900">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <script>
        document.addEventListener('livewire:init', function () {
            Livewire.on('confirmDelete', function (vendorId) {
                if (confirm('¿Está seguro de que desea eliminar este proveedor?')) {
                    window.livewire.find('@this.id').deleteVendor(vendorId);
                }
            });
        });
    </script>
