<header x-data="{ open: false }">
    <div class="flex justify-between items-center py-6 px-2 sm:pt-[2.625rem] sm:pb-5 sm:px-10">
        ...
        <div class="hidden space-x-4 sm:flex sm:items-center sm:ms-6">
            ...
            <x-dropdown align="left" width="48">
                <x-slot name="trigger">
                    <button
                        class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium leading-4 text-gray-500 transition duration-150 ease-in-out border border-transparent rounded-md hover:bg-white focus:outline-none">
                ...
                <x-slot name="content">
                    ...
                    <button onclick="confirmarSalida()" class="w-full text-start">
                        <x-responsive-nav-link>
                            Salir
                        </x-responsive-nav-link>
                    </button>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="text-base font-medium text-gray-800"
                    x-data="{{ json_encode(['name' => auth()->user()->firstName()]) }}" x-text="name"
                    x-on:profile-updated.window="name = (($event.detail.name || '').trim().split(/\s+/).filter(Boolean)[0] || '')"></div>
    ...
    @once
        <form id="logout-form-header" method="POST" action="{{ route('logout-session') }}" class="hidden">
            @csrf
        </form>
