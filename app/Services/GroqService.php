...
class GroqService
{
    ...
    public function __construct()
    {
        ...
        $this->apiKeys = array_values(array_filter([
            ...
            // config('services.groq.api_key_2'), // DEV ONLY
            // config('services.groq.api_key_3'), // DEV ONLY
            // config('services.groq.api_key_4'), // DEV ONLY
            // config('services.groq.api_key_5'), // DEV ONLY
        ]));
    ...
    public function chat(array $messages, array $tools = []): string
    {
        ...
        // Todas las keys agotadas — fallback desactivado para producción
    ...
    private function tryWithKey(string $apiKey, int $keyNumber, array $messages, array $tools): ?string
    {
        ...
        try {
            ...
            if ($response->failed()) {
                Log::error("Groq API error con key #{$keyNumber}", ['status' => $response->status(), 'body' => $response->body()]);
                return config('agent.error_messages.query_error', 'Lo siento, hubo un error al procesar tu consulta. Por favor intenta de nuevo.');
            }

            $data    = $response->json();
            $message = $data['choices'][0]['message'] ?? null;

            if (!$message) return 'No pude obtener una respuesta. Por favor intenta de nuevo.';
            ...
            return config('agent.error_messages.query_error', 'Ocurrió un error de conexión. Por favor intenta de nuevo.');
        }
    }
    ...
    private function runToolLoop(array $firstMessage, array $messages, array $tools, string $currentKey, int $keyNumber): string
    {
        ...
        while ($iteration < $this->maxToolIterations) {
            ...
            if (!isset($currentMessage['tool_calls']) || empty($currentMessage['tool_calls'])) {
                ...
                if (!empty($content)) return $this->cleanResponse($content);
                break;
            }

            $toolCalls  = $currentMessage['tool_calls'];
            $messages[] = ['role' => 'assistant', 'tool_calls' => $toolCalls];
            ...
            foreach ($toolCalls as $toolCall) {
                ...
                Log::info('[Groq] Tool ejecutada', ['tool' => $toolName, 'key' => $keyNumber, 'iteration' => $iteration, 'result' => array_keys($result)]);

                $messages[] = ['role' => 'tool', 'tool_call_id' => $toolCall['id'], 'content' => json_encode($result)];
            }
            ...
            try {
                ...
                if ($response->status() === 429) {
                    ...
                    Log::warning('Todas las keys agotadas en bucle — fallback desactivado');
                    return 'El servicio de IA no está disponible temporalmente. Por favor intenta en unos minutos.';
                }

                if ($response->failed()) {
                    Log::error('Groq API error en bucle', ['status' => $response->status(), 'key' => $keyNumber, 'iteration' => $iteration]);
                    return config('agent.error_messages.query_error', 'Lo siento, hubo un error al procesar tu consulta. Por favor intenta de nuevo.');
                }
                ...
                if (empty($currentMessage['tool_calls']) && !empty($currentMessage['content'])) {
                    Log::info('[Groq] Respuesta final obtenida', ['iteration' => $iteration, 'key' => $keyNumber]);
                    return $this->cleanResponse($currentMessage['content']);
                }

            } catch (\Exception $e) {
                Log::error('Groq runToolLoop exception', ['iteration' => $iteration, 'key' => $keyNumber, 'error' => $e->getMessage()]);
                return config('agent.error_messages.query_error', 'Ocurrió un error de conexión. Por favor intenta de nuevo.');
            }
        }
    ...
    private function getNextAvailableKey(int $currentKeyNumber): ?array
    {
        ...
        for ($i = $currentIndex + 1; $i < count($keys); $i++) {
            if (!empty($keys[$i])) return ['key' => $keys[$i], 'number' => $i + 1];
        }
    ...
    private function cleanResponse(string $content): string
    {
        ...
        $cleanLines = [];
        $debugPhrases = [
            'We need to call', 'We need to simulate', 'We need to actually call',
            'We need to get result', 'We need to wait', 'We need TEUs',
            'I need to call', 'Let me call', 'Calling tool', 'Tool call:',
            'tool_call', 'get_teus_summary', 'get_orders', 'query_operational', 'get_full_summary',
        ];

        foreach ($lines as $line) {
            $trimmed = trim($line);
            if (empty($trimmed)) { $cleanLines[] = ''; continue; }

            $isDebug = false;
            foreach ($debugPhrases as $phrase) {
                if (stripos($trimmed, $phrase) !== false) { $isDebug = true; break; }
            }

            if (str_starts_with($trimmed, '{') || str_starts_with($trimmed, '[')) $isDebug = true;
            if (!$isDebug) $cleanLines[] = $line;
        }
