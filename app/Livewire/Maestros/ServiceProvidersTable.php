...
class ServiceProvidersTable extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        if ($searchTrimmed !== '') {
            $serviceProviders = $this->fetchAllAndFilterCaseInsensitive(
                fn (array $params) => $this->getMaestrosApiService()->getServiceProviders($params),
                $searchTrimmed,
                $this->perPage,
                $this->getPage(),
                $this->sortField,
            ...
            return view('livewire.maestros.service-providers-table', [
        ...
        $response = $this->getMaestrosApiService()->getServiceProviders($params);

        if (!$response || !isset($response['data'])) {
            $serviceProviders = new LengthAwarePaginator(
                collect([]),
                0,
                $this->perPage,
                $this->getPage(),
