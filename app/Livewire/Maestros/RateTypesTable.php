...
class RateTypesTable extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        if ($searchTrimmed !== '') {
            $rateTypes = $this->fetchAllAndFilterCaseInsensitive(
                fn (array $params) => $this->getMaestrosApiService()->getRateTypes($params),
                $searchTrimmed,
                $this->perPage,
                $this->getPage(),
                $this->sortField,
            ...
            return view('livewire.maestros.rate-types-table', [
                'rateTypes' => $rateTypes
            ]);
        }
        ...
        $response = $this->getMaestrosApiService()->getRateTypes($params);

        if (!$response || !isset($response['data'])) {
            $rateTypes = new LengthAwarePaginator(
                collect([]),
                0,
                $this->perPage,
                $this->getPage(),
